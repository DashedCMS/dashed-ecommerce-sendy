...
class KeendeliverySettingsPage extends Page implements HasForms
{
    ...
    protected function getFormSchema(): array
    {
        ...
        foreach ($sites as $site) {
            $schema = [
                ...
                Placeholder::make('label')
                    ->label("KeenDelivery is " . (! Customsetting::get('channable_connected', $site['id'], 0) ? 'niet' : '') . ' geconnect')
                ...
                TextInput::make("channable_company_id_{$site['id']}")
                    ->label('KeenDelivery company ID')
                    ->rules([
                        'max:255',
                    ]),
                TextInput::make("channable_project_id_{$site['id']}")
                    ->label('KeenDelivery project ID')
                    ->rules([
                        'max:255',
                    ]),
                Toggle::make("channable_feed_enabled_{$site['id']}")
                    ->label('KeenDelivery feed aanzetten'),
                Toggle::make("channable_order_sync_enabled_{$site['id']}")
                    ->label('Order uit KeenDelivery naar webshop syncen'),
                Toggle::make("channable_stock_sync_enabled_{$site['id']}")
                    ->label('Voorraad vanuit webshop naar KeenDelivery syncen'),
            ];
    ...
    public function submit()
    {
        ...
        foreach ($sites as $site) {
            ...
            Customsetting::set('channable_connected', KeenDelivery::isConnected($site['id']), $site['id']);
        }
